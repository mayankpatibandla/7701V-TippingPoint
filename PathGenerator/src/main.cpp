...
//be0e06de00e07db66f97686505c3f4dde2e332dc.pdf
//PART 3 CALUCLATIONS
...
void purePursuit(){
  ...
  double spacing = 40;
  double weightSmooth = 0.65, weightData = 1-weightSmooth,  tolerance = 0.00001;
  double lookAheadDistance = 15;
  double trackWidth = 30;
  double k = 1.5, maxVelocity = 65, maxAcceleration = 10;
  double maxRate = 10;
  double kV = 1/maxVelocity, kA = 0.002, kP = 0.01;
  ...
  Path injectedPath = PathGenerator::injectPoints(waypointPath, spacing);
  Brain.Screen.clearScreen();
  ...
  while(true){
    ...
    //input
    double speedReduction = 95;
    robot.position.center.x += Controller.Axis4.position() / speedReduction;
    robot.position.center.y -= Controller.Axis3.position() / speedReduction;

    robot.theta = -std::atan2(Controller.Axis2.position(), Controller.Axis1.position());

    if(Controller.ButtonY.pressing()) goto resetSimulation;
    ...
    //calculate and drawsigned curvature
    ...
    //std::cout << "LEFT: " << leftSideFinalVel << "\t\t\t\t" << "RIGHT: " << rightSideFinalVel << std::endl;
    ...
    //reversed due to screen coords having flipped y-axis; in final version they shouldn't be
    Brain.Screen.printAt(0, 40, "Left Vel: %lf, Right Vel: %lf", rightSideFinalVel, leftSideFinalVel);
