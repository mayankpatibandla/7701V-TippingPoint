...
namespace PositionTracking{
  ...
  const double s_L = 5/16;
  const double s_R = 1/0.;
  ...
  void positionTracking(){
    ...
    while(true){
      ...
      //Equation is circumference of wheel times section of circle where the encoder's angle changed:
      ...
      //get robot heading from tracking wheels instead because it is much more accurate
      else{
      ...
      if(delta_theta == 0){
        ...
        delta_d_l_y = 2 * std::sin(delta_theta / 2) * (delta_L / delta_theta - s_L);
      }
