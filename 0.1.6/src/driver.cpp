...
void driver(){
  ...
  while(true){
    //auton test
    if(Controller.ButtonRight.pressing())auton();
    ...
    if(Controller.ButtonL1.pressing() && !fourBarTopLimit.pressing()){
    ...
    if(Controller.ButtonR1.pressing() && !backLiftTopLimit.pressing()){
      backLiftMotor.spin(fwd, 100, pct);
      backLift = false;
    }
    else if(Controller.ButtonR2.pressing() && !backLiftBottomLimit.pressing()){
      backLiftMotor.spin(fwd, -100, pct);
      backLift = false;
    }
    else if(backLift){
      ...
      if(backLiftBottomLimit.pressing()) pow = 0;
