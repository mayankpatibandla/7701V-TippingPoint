...
rotation rightRotationSensor(PORT6);
rotation backRotationSensor(PORT14);

motor frontLeftMotor(PORT1, false);
motor frontRightMotor(PORT10, true);
motor backLeftMotor(PORT20, false);
motor backRightMotor(PORT9, true);

motor leftFourBarMotor(PORT2, ratio36_1, true);
motor rightFourBarMotor(PORT8, ratio36_1);

motor backLiftMotor(PORT3, ratio36_1, true);

motor ringLiftMotor(PORT4, ratio6_1, true);

inertial inertialSensor(PORT13);

distance clawDistanceSensor(PORT12);
optical clawOpticalSensor(PORT15);
...
limit backLiftBottomLimit(Brain.ThreeWirePort.F);
limit backLiftTopLimit(Brain.ThreeWirePort.E);
...
int backLiftLimit(){
  while(true){
    uint32_t timeStart = Brain.Timer.system();
    if(backLiftBottomLimit.pressing()){
      if(backLiftMotor.velocity(pct) < 0){
        backLiftMotor.setVelocity(0, pct);
        backLiftMotor.spin(fwd);
      }
    }
    if(backLiftTopLimit.pressing()){
      if(backLiftMotor.velocity(pct) > 0){
        backLiftMotor.setVelocity(0, pct);
        backLiftMotor.spin(fwd);
      }
...
task backLiftLimit_task(backLiftLimit);
