...
namespace PurePursuit{
  ...
  void PurePursuit::setPath(std::vector<PathPoint> &path){
    this->path = path;
  }

  void PurePursuit::setlookDistance(double lookDistance){
    this->lookDistance = lookDistance;
  }

  void PurePursuit::setRobotTrack(double robotTrack){
  ...
  void PurePursuit::update(){
    ...
    drawClosest(bot.getCanvasPos(), closestPointVector);
    drawCurvature(curvature, bot.getLocalPos(), finalLookPoint);
    */
    bot.draw();
  };

  int PurePursuit::findClosestIndex(Vector &currentPos){
  ...
  double PurePursuit::findIntersectT(Vector &segmentStart, Vector &segmentEnd, Vector &currentPos){
  ...
  Vector PurePursuit::findLookahead(Vector &currentPos){
  ...
  double PurePursuit::findLookaheadCurvature(Vector &currentPos, double heading, Vector &lookPoint){
  ...
  double PurePursuit::computeLeftVel(double targetVel, double curvature){
    return targetVel * (2 + robotTrack * curvature) / 2;
  }

  double PurePursuit::computeRightVel(double targetVel, double curvature){
    return targetVel * (2 - robotTrack * curvature) / 2;
  }
}